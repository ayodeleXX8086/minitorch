...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    s = sigmoid(a)
    assert 0.0<=s<=1.0,  f"Sigmoid({a})={s} not in (0,1)"

    sig_neg = sigmoid(-a)
    assert_close(1-s, sig_neg), f"Expected {1-s} to be same as {sig_neg}"

    if a==0.0:
        assert abs(s-0.5)<1e-6, f"Sigmoid(0)={s} != 0.5"
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if a < b < c:
        assert (a < b < c and a < c), f"Transitivity failed: {a} < {b} and {b} < {c}, but {a} !< {c}"
...
def test_symmetric() -> None:
    ...
    for x, y in test_values:
        assert_close(mul(x, y), mul(y, x)), f"mul({x},{y}) != mul({y},{x})"
...
def test_distribute() -> None:
    ...
    for z, x, y in test_values:
        left = mul(z, add(x, y))
        right = add(mul(z, x), mul(z, y))
        assert_close(left, right), f"{z}*({x}+{y}) != {z}*{x} + {z}*{y}"
